var check = {
    checkImg: function () {
        ...
        if (pimg === null || pimg === "") {
            $(".spanImg").html("<img src='/img/error.png'>请选择图片").css("color", "red");
            return false;
        }
        if (!reg.test(pimgSuf)) {
            $(".spanImg").html("<img src='/img/error.png'>图片格式错误").css("color", "red");
            return false;
        } else {
            $(".spanImg").html("<img src='/img/green.png'>").css("color", "green");
            return true;
        }
    },
    ...
    checkName: function () {
        ...
        if (nameText === null || nameText === "") {
            $(".span:first").html("<img src='/img/error.png'>请填写商品名称").css("color", "red");
            return false;
        } else {
    ...
    checkPrice: function () {
        ...
        if (priceText === null || priceText === "") {
            $(".span:eq(1)").html("<img src='/img/error.png'>请填写商品价格").css("color", "red");
            return false;
        }
        if (price <= 0 || isNaN(price)) {
            $(".span:eq(1)").html("<img src='/img/error.png'>商品价格错误，请输入大于0的数字").css("color", "red");
            return false;
        } else {
    ...
    checkDiscount: function () {
        ...
        if (discount <= 0 || discount > 1 || isNaN(discount)) {
            $(".span:eq(2)").html("<img src='/img/error.png'>商品折扣范围在0~1").css("color", "red");
            return false;
        } else {
    ...
    checkStock: function () {
        ...
        if (stockText === null || stockText === "") {
            $(".span:eq(3)").html("<img src='/img/error.png'>请填写商品库存").css("color", "red");
            return false;
        }
        if (stock < 0) {
            $(".span:eq(3)").html("<img src='/img/error.png'>请输入不小于0的数字").css("color", "red");
            return false;
        } else {
    ...
    checkDesc: function () {
        ...
        if (pdscText === null || pdscText === "") {
            $(".span:eq(4)").html("<img src='/img/error.png'>请填写商品描述").css("color", "red");
            return false;
        } else {
